...
var cloneDeep = (v, refs) => {
  if (isNil(v) || isBoolean(v) || isNumber(v) || isString(v))
    return v;
  if (isDate(v))
    return new Date(v);
  if (isRegExp(v))
  ...
  try {
    if (isArray(v)) {
      ...
      for (let i = 0; i < v.length; i++)
        arr[i] = cloneDeep(v[i], refs);
      return arr;
    }
    if (isObject(v)) {
      const obj = {};
      for (const k of Object.keys(v))
        obj[k] = cloneDeep(v[k], refs);
      return obj;
    }
  } finally {
    refs.delete(v);
...
function merge(target, input) {
  if (isMissing(target) || isNil(target))
    return input;
  if (isMissing(input) || isNil(input))
    return target;
  if (isPrimitive(target) || isPrimitive(input))
    return input;
...
var stringify = (v, refs) => {
  ...
  try {
    refs.add(v);
    if (isArray(v))
      return "[" + v.map((s2) => stringify(s2, refs)).join(",") + "]";
    if (isObject(v)) {
...
var _Context = class _Context {
  ...
  addOperators(type, operators) {
    if (!__privateGet(this, _operators).has(type))
      __privateGet(this, _operators).set(type, {});
...
function computeExpression(obj, expr, options) {
  ...
  if (isString(expr) && expr.length > 0 && expr[0] === "$") {
    if (REDACT_ACTIONS.includes(expr))
      return expr;
...
function computeOperator(obj, expr, operator, options) {
  ...
  if (callExpression)
    return callExpression(obj, expr, options);
...
var Iterator = class {
   ...
   */
  constructor(source) {
    __privateAdd(this, _iteratees, void 0);
    __privateAdd(this, _yieldedValues, void 0);
    __privateAdd(this, _getNext, void 0);
  ...
  for (const key of expressionKeys) {
    ...
    if (isNumber(subExpr) || isBoolean(subExpr)) {
      ...
      if (projectFn) {
        ...
        handlers[key] = (o) => {
          if (!has(o, key))
            return computeValue(o, subExpr, null, options);
          if (isRoot)
            options.update(o);
...
var Cursor = class {
  ...
  fetch() {
    ...
    if (mode & ProcessingMode.CLONE_INPUT)
      __privateGet(this, _result).map(cloneDeep);
...
var Query = class {
  constructor(condition, options) {
    ...
    __privateAdd(this, _condition, void 0);
...
//# sourceMappingURL=chunk-4H7V2DEK.js.map
