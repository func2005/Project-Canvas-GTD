...
| **system_status**         | String    | `'active'`, `'completed'`, `'dropped'`, `'archived'`。       |
| **title**                 | Text      | 事务标题。                                                   |
...
### 3.2 视图配置层 (Presentation Layer) - The Canvas
...
| **widget_type**        | String | `'calendar_master'`, `'smart_list'`, `'matrix'`, `'detail'`。 |
...
## 4. 画布组件体系 (Canvas Components) [完整实施版]

在本项目中，所有功能模块均被封装为**独立的、可实例化的浮动窗口 (Floating Widgets)**。它们的数据状态持久化于 `canvas_widgets` 表中，通过 RxDB 在客户端实时渲染。

### 4.1 组件通用物理属性 (Common Physics)

所有组件实例（无论是日历还是列表）都共享一套基础的“外框”交互逻辑与数据结构：

-   **Header Bar (标题栏)**：
    -   **Title**：显示当前上下文（如 "Linked: Nov 20" 或 "Project: Apollo"）。
    -   **Pin Button (📌)**：修改 `view_state.is_pinned`。锁定后，组件不再响应上游信号，`data_source_config` 被固化。
    -   **Link Handle (🔗)**：用于发起“新建关联组件”的操作。
...
### 4.2 组件实例数据模型 (The Widget Schema)
...
interface CanvasWidget {
  ...
  view_state: {
    ...
    is_collapsed: boolean;        // 是否最小化
    view_mode: string;            // e.g., 'month', 'week', 'kanban'
  };
}
```

### 4.3 核心组件详解 (Core Widgets)

#### A. 主控日历 (Master Calendar)
...
#### C. 艾森豪威尔矩阵 (The Matrix)

-   **Type**: `matrix`
-   **数据逻辑**: 获取所有 `system_status='active'` 且 `type='task'` 的项。
-   **动态映射 (The Magic)**:
    -   **X轴 (紧急度)**: 前端动态计算 `1 / (due_date - today)`。无死线任务停留在最左侧缓冲区。
    -   **Y轴 (重要性)**: 映射 `properties.priority` (High=Top, Normal=Mid, Low=Bottom)。
-   **交互**: **拖拽即修改**。将任务从 Q4 拖入 Q1，自动更新 `priority='high'` 和 `do_date=today`。

#### D. 详情检查器 (Detail Inspector)
...
### 4.4 复合与特殊组件 (Compound & Special Widgets)

这些组件在逻辑上更复杂，可能由多个 Widget 组合而成。

#### E. 项目组合视图 (Project Hub) —— *[v4.2 核心特性复原]*

在 v6.0 架构中，"Project Hub" 不是一个单一的大组件，而是一组**预设布局的组件群**。

-   **触发**: 用户右键项目 $\rightarrow$ "Open on Canvas"。
-   **实例化过程**: 系统一次性在 `canvas_widgets` 表中创建以下 3 个组件，并设定它们的位置相邻：
    1.  **Header Widget**: 显示项目标题、进度条、Markdown 简介 (`type='project_header'`)。
    2.  **Project Calendar**: 仅显示该项目 ID 下的 deadlines (`type='calendar_master', config={project_id: '...'}`)。
    3.  **Project List**: 显示该项目的下一步行动 (`type='smart_list', config={project_id: '...'} `)。
-   **Group Behavior**: 前端实现“成组拖拽”，移动 Header 时带动其他两个组件。

#### F. 归档与收集箱 (System Bins)
...
-   **Archive (归档箱)**
    ...
    -   **物理交互**:
        -   **Delete**: 将任何 Item 拖入此组件，触发 `update({ system_status: 'completed' })` 并伴随粉碎动画。
        -   **Lazy Load**: 展开列表时，触发服务端 SQL 查询（冷数据不全量同步到 RxDB）。


-----

## 5. 交互与信号流转逻辑 (Interaction & Signal Flow) 
...
### 5.1 信号源与响应规则 (Signal Source & Response Map)

组件间的联动本质上是上游组件动态改写下游组件的 `JSON Query`。

| 上游操作 (Source Action)                                     | 信号含义 (Payload)                            | 下游组件响应 (Downstream RxDB Query Update)                  | 业务场景                                           |
| :----------------------------------------------------------- | :-------------------------------------------- | :----------------------------------------------------------- | :------------------------------------------------- |
...
### 5.2 画布视觉反馈 (Spatial Visual Feedback)
...
### 5.3 锁定与快照机制 (Pinning & Snapshot)

这是解决“数据修改后消失”问题的核心逻辑。

  * **状态 A: 订阅模式 (Linked/Unpinned)**

    * `view_state.is_pinned = false`。
    * **行为**: 下游组件实时监听上游信号。
    * *例子*: 日历选“今天”，列表显示“今天”。日历切“明天”，列表变“明天”。

  * **状态 B: 锁定模式 (Pinned/Snapshot)**
...
### 5.4 空间拖拽逻辑 (Spatial Drag & Drop)

拖拽是修改 `data_items` 属性最高效的方式。以下表格定义了拖拽事件触发的 RxDB 更新逻辑。

| 拖拽源 (Source)   | 放置目标 (Drop Target)   | 业务语义                  | 底层数据变更 (RxDB Update)                                   |
| :---------------- | :----------------------- | :------------------------ | :----------------------------------------------------------- |
...
| **Any Item**      | **Archive Box**          | **完成/归档**             | `doc.atomicPatch({ system_status: 'completed', completed_at: now() })` |

### 5.5 关联组件创建流 (Link Creation Flow)

替代手动连线，通过交互生成关联数据。

1.  **用户意图**：在“主控日历”上选中了 11月20日，想固定查看当天的详情。
2.  **操作**：点击日历标题栏上的 **[➕ Link]** 按钮，选择 "Create List"。
3.  **系统响应 (事务性操作)**：
      * **Step 1 (Widget)**: 创建新组件记录 `Widget_B` (`type='smart_list'`)。
      * **Step 2 (Link)**: 创建连线记录 `Link_AB` (`source=Calendar_A, target=Widget_B`)。
      * **Step 3 (Init)**: 读取 Calendar\_A 当前选中的日期 (Nov 20)，初始化 Widget\_B 的 `data_source_config` 为 `{ do_date: '2025-11-20' }`。
4.  **结果**：一个显示着 11月20日 任务的新列表出现在日历旁，且两者已建立逻辑连接。


-----

## 6\. 自动化与业务规则 (Automation & Business Logic)
...
### 6.1 时间维度的自动化 (Temporal Automation)

处理时间流逝带来的状态变更，核心目标是\*\*“消除过期焦虑”\*\*。

#### A. 逾期幽灵视图 (Overdue Ghost Projection) —— *[Client-Side RxDB]*
  ...
  * **视觉表现**: 幽灵任务 (Ghosts) 强制置顶，背景带有红色半透明纹理，显示标签 "3 days ago"。

#### B. 智能逾期警告 (Smart Overdue Alert) —— *[UI Computed]*
...
#### C. 紧急度动态评分 (Dynamic Urgency Score) —— *[Matrix Widget Logic]*

  * **用途**: 决定艾森豪威尔矩阵中任务在 X 轴（紧急度）的物理坐标。
  * **算法**:
    * $DaysLeft = due\_date - today$
    * $X\_Position = \text{Clamp}( \frac{1}{DaysLeft}, \ 0, \ 100 )$
  * **动态效果**: 随着时间推移（$DaysLeft$ 变小），未完成的任务会在矩阵视图中**自动向右漂移**，最终越过中线变为“紧急”。

### 6.2 结构维度的自动化 (Structural Automation)
...
#### B. 项目自动完成建议 (Project Completion Prompt) —— *[Client-Side Logic]*
...
### 6.3 属性继承与衍生逻辑 (Inheritance & Derivation)

减少用户输入，通过上下文自动填充 `properties`。

#### A. 从日程衍生任务 (Event-to-Task)

  * **场景**: 在日历的 Event 上右键选择 "Create Prep Task" (创建准备任务)。
  * **自动填充规则**:
    1.  `parent_id`: 继承 Event 的 ID (或 Event 所属的 Project ID)。
    2.  `properties.tags`: 继承 Event 的 Tags (如 `@Office`)。
    3.  `do_date`: 默认为 Event **开始前一天**。
    4.  `due_date`: 严格设为 Event 的 **start\_time**。

#### B. 矩阵拖拽映射 (Matrix Dragging Logic)

当任务被拖入矩阵的不同象限时，自动修改属性：

| 拖入区域 (Quadrant) | 自动修改 (Auto-Update RxDB)                            | 业务含义                                 |
| :------------------ | :----------------------------------------------------- | :--------------------------------------- |
| **Q1 (右上)**       | `properties.priority`='high', `do_date`=Today          | **立刻做** (Important & Urgent)          |
| **Q2 (左上)**       | `properties.priority`='high', `do_date`=Tomorrow+      | **规划做** (Important & Not Urgent)      |
| **Q3 (右下)**       | `properties.priority`='low', `do_date`=Today           | **授权/快速做** (Not Important & Urgent) |
| **Q4 (左下)**       | `properties.priority`='low', `system_status`='dropped' | **放弃/将来做** (Waste)                  |

### 6.4 上下文感知建议 (Context-Aware Suggestions)

#### A. 晚间低能耗模式 (Evening Low-Energy Mode) —— *[View Layer]*

  * **触发条件**: 客户端系统时间 \> 20:00 (用户可配置)。
  * **逻辑**: 系统尝试降低“高耗能任务”的视觉权重，防止晚间焦虑。
  * **渲染行为**:
    * 遍历当前列表。
    * IF `item.properties.energy_level == 'high'` THEN `opacity: 0.5` (变暗/半透明)。
    * 同时高亮 `properties.energy_level == 'low'` 的任务（如“整理桌面”、“阅读”）。


-----

## 7. 数据查询接口规范 (Data Query Interface Specs)
...
### 7.3 艾森豪威尔矩阵 (Matrix Query) —— *[Client: RxDB]*

矩阵视图获取所有活跃任务，坐标 (`x, y`) 由前端组件**后置计算**。
...
```
  ...
  * **前端计算逻辑 (UI Layer)**:
    * $X = f(\text{due\_date} - \text{now})$
    * $Y = \text{properties.priority}$

### 7.4 项目组合视图 (Project Hub) —— *[Client: RxDB]*

当用户“展开”一个项目时，这组查询由 **Project Hub** 的三个子组件分别执行。

```javascript
// 1. 项目头信息 (Header Widget)
const projectInfoQuery = {
  selector: { id: $projectId }
};

// 2. 项目内统计 (Reactive Count)
// RxDB 支持 .count() 订阅
const progressQuery = {
  selector: { parent_id: $projectId, system_status: 'completed' }
}; 

// 3. 项目任务列表 (List Widget)
const projectTasksQuery = {
  selector: {
    parent_id: $projectId,
    system_status: { $ne: 'archived' }
  },
  sort: [{ sort_order: 'asc' }, { due_date: 'asc' }]
};
```

### 7.5 服务端同步与守护接口 (Server-Side SQL)
...
### 7.6 全局归档检索 (Global Archive Search) —— *[Server API]*
...
## 8. UI 组件规范与交互详解 (UI Specifications)

本章节定义了前端组件的详细交互行为。在 Local-First 架构下，所有交互必须遵循 **“立即反馈，后台同步”** 的原则。

### 8.1 通用外框控制器 (Common Frame Controls)

所有组件实例（`canvas_widgets`）均包裹在一个统一的容器 (Wrapper) 中，该容器负责处理物理属性。
...
| **Add Link**<br>(🔗 / ➕ 图标)      | 单击               | **新建关联 (Transaction)**：<br>1. 弹出菜单选择下游类型 (e.g., "Linked List")。<br>2. **DB Action**: 并在当前组件右侧 `x+width+20` 处创建一个新 Widget 记录。<br>3. **Link Action**: 创建一条 `canvas_links` 记录。<br>4. **Visual**: 自动绘制连接线。 |
| **Close**<br>(✖ 图标)             | 单击               | **销毁**：<br>执行 `widget.remove()`。这仅删除视图实例，**绝不删除**业务数据 (`data_items`)。 |

---

### 8.2 智能列表视图 (Smart List View)
...
* **[Energy Filter]**: 切换按钮 `[All | 🧠 | ☕️]`。修改 RxDB Query 的 `selector`。
* **[+ Add Task]**:
    * **Action**: 在列表顶部插入一个临时的 Input Row。
...
| 区域/元素                   | 动作     | 交互细节与数据变更                                           |
| :-------------------------- | :------- | :----------------------------------------------------------- |
| **Checkbox**                | 单击     | **乐观完成 (Optimistic Completion)**：<br>1. **UI**: 立即播放打钩动画，文字变灰划掉。<br>2. **DB**: `doc.atomicPatch({ system_status: 'completed', completed_at: now() })`。<br>3. **Delay**: 延迟 800ms 后，该行从视图中平滑移除 (Layout Shift)。 |
| **Title Text**              | 单击     | **选中信号**：<br>1. **UI**: 该行高亮。<br>2. **Signal**: 广播 `{ type: 'item', id: doc.id }`。如果画布上有未锁定的 Detail Widget，它会立即显示该任务详情。 |
| **Title Text**              | 双击     | **行内编辑**：<br>文本变为 `<input>` 框。Blur 或 Enter 时自动保存 `title`。 |
| **Drag Grip**<br>(::before) | 拖拽开始 | **启动搬运**：<br>1. 生成半透明 Ghost Image。<br>2. 画布上所有可接受 Drop 的区域（日历格、矩阵象限、归档箱）高亮显示 (Drop Zones Highlight)。 |

---

### 8.3 主控日历 (Master Calendar)
...
### 8.4 艾森豪威尔矩阵 (The Matrix)

矩阵本质上是一个**可视化的属性修改器**。

| 区域 (Quadrant) | 接收 Drop 后的数据变更逻辑                           |
| :-------------- | :--------------------------------------------------- |
| **Q1 (右上)**   | `priority: 'high', do_date: today` (立即做)          |
| **Q2 (左上)**   | `priority: 'high', do_date: tomorrow+` (计划做)      |
| **Q3 (右下)**   | `priority: 'low', do_date: today` (凑合做)           |
| **Q4 (左下)**   | `priority: 'low', system_status: 'dropped'` (不做了) |

* **交互反馈**: 当 Item 被拖到某象限上方时，该象限背景变色 (Hover State)，提示用户即将发生的变更后果。

---

### 8.5 详情检查器 (Detail Inspector)
...
### 8.7 项目组合视图 (Project Hub Macro)

这是特殊的**“组交互”**规范。

* **成组选择 (Group Selection)**: 点击 Project Header 组件时，关联的 Project Calendar 和 Project List 会显示“被关联”的虚线框。
* **成组拖拽 (Group Drag)**: 拖动 Project Header 时，其他两个组件会保持相对位置跟随移动 (就像成组的图形对象一样)。
* **折叠 (Collapse)**: 点击 Header 上的折叠按钮，Calendar 和 List 会收缩并隐藏，只保留 Header 条，节省画布空间。

-----

## 9. 用户旅程与冷启动 (User Journey & Cold Start)

本章节描述用户首次进入系统时的默认状态，以及完成第一个核心闭环（Capture $\rightarrow$ Schedule $\rightarrow$ Review $\rightarrow$ Focus）的标准剧本。

### 9.1 默认初始化状态 (The Default Layout)
...
### 9.2 场景 A: 捕获与排程 (Capture & Schedule)
...
### 9.3 场景 B: 详情查看与“窗口复用”策略 (Window Reuse)
...
### 9.4 场景 C: 锁定与多窗口并行 (Pinning & Multi-Window)
...
1.  **锁定上下文**:
    ...
    * **UI 反馈**: 边框变色，标题栏显示 "Fixed: 阅读"。此时它切断了与日历的信号监听。
...
### 9.5 场景 D: 深度钻取与三角关联 (Drill Down & Context Triangle)
...
1.  **展开某日**:
    ...
    * **系统响应**:
        * 创建关联组件 `Widget D (Smart List)`。
        * 建立连接: `Link: Calendar -> Widget D`。
...
2.  **三角联动验证**:
    ...
    * **逻辑总结**: 只要组件未锁定，它就是整个画布数据的**“公共投影仪”**，任何上游组件的点击都会将内容投射到它上面。要变更点包括：

1.  **架构升级**：引入“Local-First”理念，明确了数据（Data）与视图（Canvas）的物理分离。
2.  **数据模型重构**：将单一的 `Items` 表拆解为业务核心 `data_items` 和画布配置 `canvas_widgets`。
3.  **属性扩展**：将 Priority, Energy, Tags 等字段移入 `properties` JSONB 结构，以适应动态业务需求。
4.  **查询规范更新**：重新定义了基于 PostgreSQL (服务端) 和 RxDB (客户端) 的双层数据流。
