...
export const ProjectHeader: React.FC<ProjectHeaderProps> = ({ widget }) => {
    ...
    // Query project tasks for progress (only if projectId exists)
    const tasks = useRxQuery<DataItem>('items', projectId ? {
        selector: {
            entity_type: 'task',
        ...
        selector: {
            id: 'non-existent' // Return empty results if no projectId
        }
    });
